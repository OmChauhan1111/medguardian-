...
# ---------- Settings ----------
# None => auto-logout disabled (jab tak browser session hai, login rahega)
TIMEOUT_MINUTES = None  # 15  # auto-logout after this many minutes of inactivity (set None to disable)
...
def check_auto_logout():
    ...
    if st.session_state.get("user") and st.session_state.get("last_active"):
        ...
        if elapsed > TIMEOUT_MINUTES * 60:
            ...
            st.warning(
                f"Session timed out after {TIMEOUT_MINUTES} minutes of inactivity. Please login again."
            )
            safe_rerun()
...
def download_model_from_gdrive(file_id: str, out_path: str, quiet=False):
    ...
    try:
        ...
        if not quiet:
            print(
                "gdown not installed - skipping auto-download. Install with `pip install gdown` to enable auto-download."
            )
        return False
    ...
    try:
        ...
        if os.path.exists(out_path) and os.path.getsize(out_path) > 200:
            if not quiet:
                print(
                    f"[MODEL DOWNLOAD] Download complete: {out_path} ({os.path.getsize(out_path)} bytes)"
                )
            return True
        else:
            if not quiet:
                print(
                    f"[MODEL DOWNLOAD] Downloaded file seems too small or missing after download: {out_path}"
                )
            return False
    ...
    try:
        ...
        with open(path, "rb") as f:
            ...
            if head.startswith(b"\x80") or head.startswith(b"\x93") or head.startswith(b"\x01"):
                return True
            if head.startswith(b"<!DO") or head.startswith(b"{") or head.startswith(
                b"<htm"
            ) or head.startswith(b"<?xm"):
                print(
                    f"[MODEL LOAD] {path} looks like text/HTML/JSON rather than a binary pickle."
                )
                return False
            return True
    except Exception as e:
...
def _safe_load(path):
    ...
    try:
        ...
        raise RuntimeError(
            f"Failed to load model file {path}: joblib error: {e_job}; pickle error: {e_pickle}"
        )
        ...
        try:
            if os.path.exists(p):
                ...
                if _validate_model(obj, name=key):
                    models[key] = obj
                    st.sidebar.success(
                        f"{key.capitalize()} model loaded ✅ ({type(obj).__name__})"
                    )
                else:
                    models[key] = None
                    st.warning(
                        f"Loaded file for {key} but it's not a valid model (missing .predict). Using fallback (disabled)."
                    )
            else:
                models[key] = None
                st.sidebar.error(f"{key.capitalize()} model file not found: {p}")
        except Exception as e:
            models[key] = None
...
def show_auth_page():
    ...
    with col_logo:
        ...
        st.markdown(
            """
            <h3 style='text-align:center; margin-top:8px'>MedGuardian — Sign in to continue</h3>
            <p class='small-muted' style='text-align:center;'>
                Secure access to your AI health reports and chat.<br>
                ...
                if login:
                    if not login_user or not login_pw:
                        ...
                        if u:
                            ...
                            update_last_active()
                            st.success(
                                f"Welcome, {u.get('full_name') or u.get('username')}"
                            )
                            # Streamlit form submit ke baad khud rerun hota hai,
                            # isliye yahan safe_rerun() ki zarurat nahi hai
                        else:
...
with st.sidebar:
    sidebar_logo = "logo.png" if os.path.exists("logo.png") else (
        "logo_splash.png" if os.path.exists("logo_splash.png") else None
    )
    if sidebar_logo:
...
if page == "🏠 Dashboard":
    update_last_active()
    st.markdown(
        "<h2 style='text-align:center;color:#0066cc;'>🏥 MedGuardian Analytics Dashboard</h2>",
        unsafe_allow_html=True,
    )
    st.markdown(
        "<p style='text-align:center;opacity:0.7;'>Patient Health Report Center</p>",
        unsafe_allow_html=True,
    )
    ...
    for r in st.session_state.reports:
        if not any(
            (
                r.get("Patient ID") == rr.get("Patient ID")
                and r.get("Date") == rr.get("Date")
                and r.get("Condition") == rr.get("Condition")
            )
            for rr in reports
        ):
            r["__db_id"] = None
    ...
    if not reports:
        ...
        if patient_filter != "All":
            filtered = [
                r for r in filtered if r.get("Patient Name", "Unknown") == patient_filter
            ]
        ...
        if not filtered:
            ...
            for i, r in enumerate(filtered):
                ...
                with st.expander(
                    f"{icon} {condition} — {name} | Risk: {risk}%", expanded=False
                ):
                    st.markdown(
                        f"""
                    ...
                    with cols[0]:
                        try:
                            ...
                            with open(p, "rb") as f:
                                st.download_button(
                                    f"📄 Download {condition} Report",
                                    f,
                                    file_name=f"{name}_{condition}_Report.pdf",
                                    key=f"pdf_{i}_{int(time.time())}",
                                )
                        except Exception as e:
                    ...
                    with cols[1]:
                        st.download_button(
                            ...
                            file_name=f"{name}_{condition}_report.csv",
                            key=f"csv_{i}_{int(time.time())}",
                        )
                    with cols[2]:
                    ...
                    if (
                        cand
                        and cand.get("pid") == pid
                        and cand.get("condition") == condition
                        and cand.get("date") == r.get("Date")
                    ):
                        st.warning("You are about to delete this record. This action cannot be undone.")
                        c1, c2 = st.columns([1, 1])
                        if c1.button(
                            "Confirm Delete",
                            key=f"confirm_del_{dbid if dbid is not None else 'local'}_{i}",
                        ):
                            removed_db = False
                            ...
                            if cand.get("db_id") and db_delete_report is not None:
                                try:
                                    ...
                                    if ok:
                                        ...
                                        st.warning(
                                            "⚠️ Database reported 0 rows affected (no deletion)."
                                        )
                                except Exception as e:
                                ...
                                st.info(
                                    "No DB delete helper available or record is unsaved (local-only)."
                                )
                            ...
                            try:
                                ...
                                st.session_state.reports = [
                                    rr
                                    for rr in st.session_state.get("reports", [])
                                    if not (
                                        rr.get("Patient ID") == cand.get("pid")
                                        and rr.get("Condition") == cand.get("condition")
                                        and rr.get("Date") == cand.get("date")
                                    )
                                ]
                                ...
                                if after < before:
                                    ...
                                    st.warning(
                                        "⚠️ Report not found in local session list to remove."
                                    )
                            except Exception as e:
                        ...
                        if c2.button(
                            "Cancel",
                            key=f"cancel_del_{dbid if dbid is not None else 'local'}_{i}",
                        ):
                            st.session_state.pop("delete_candidate", None)
                            safe_rerun()
            ...
            if "Patient Name" not in df.columns:
                df["Patient Name"] = df.get(
                    "Patient Name", pd.Series(["Unknown"] * len(df))
                )
            ...
            fig.update_traces(
                textposition="outside", marker_line_width=0.8, marker_line_color="rgba(0,0,0,0.12)"
            )
            fig.update_layout(
                yaxis_title="Risk %",
                xaxis_title="Patient",
            ...
            st.download_button(
                "💾 Download All Filtered Reports (CSV)",
                csv,
                "All_Reports.csv",
...
# -------------------- HEALTH SCAN --------------------
    ...
    with colA:
        ...
        st.caption(
            "🔒 Patient Name and Patient Contact are required — they are used for record-keeping in the report."
        )
    ...
    with colB:
        ...
        if doctor_name == "Other":
            doctor_name_custom = st.text_input(
                "Enter Doctor Name (Other)",
                st.session_state.get("doctor_name", ""),
                help="If your doctor is not in the list, type the name here.",
            )
            doctor_name = (
                doctor_name_custom.strip() if doctor_name_custom.strip() != "" else "Other"
            )
    ...
    if disease == "Heart":
        ...
        if st.button("🔍 Predict Heart Risk"):
            if heart_model is None:
                st.error(
                    "Heart model load नहीं हुआ 😔. 'Heart/heart_model.pkl' file ka path aur file sahi hai ya nahi check karo."
                )
                st.stop()
            ...
            report = {
                ...
                "Sample Collected": st.session_state.get(
                    "sample_collected", datetime.now().strftime("%d-%m-%Y %I:%M %p")
                ),
                "Report Generated By": st.session_state.get(
                    "report_generated_by", "MedGuardian AI Lab System"
                ),
            ...
            existing_keys = {
                (r.get("Patient ID"), r.get("Condition"), r.get("Date"))
                for r in st.session_state.reports
                if r.get("Patient ID")
            }
    ...
    if disease == "Diabetes":
        ...
        if st.button("🔍 Predict Diabetes"):
            if diabetes_model is None:
                st.error(
                    "Diabetes model load नहीं हुआ 😔. 'Diabetes/Diabetes_model.pkl' file ka path aur file sahi hai ya nahi check karo."
                )
                st.stop()
    ...
    if disease == "Kidney":
        ...
        conv = (
            lambda x: 1
            if str(x).strip().lower() in ["yes", "poor", "abnormal", "1", "true"]
            else 0
        )
        df = pd.DataFrame(
            [
                {
                    ...
                    "diabetes_mellitus": conv(diabetes),
                    "coronary_artery_disease": conv(cad),
                    "appetite": conv(appetite),
                    "peda_edema": conv(edema),
                    "aanemia": conv(aanemia),
                }
            ]
        )
        st.caption(
            "Each field has a help tooltip — hover or tap to read meaning and expected ranges."
        )
        if st.button("🔍 Predict Kidney Disease"):
            if kidney_model is None:
                st.error(
                    "Kidney model load नहीं हुआ 😔. 'Kidney/kidney_model.pkl' file ka path aur file sahi hai ya nahi check karo."
                )
                st.stop()

            if not validate_patient_details():
                st.stop()
            res = kidney_model.predict(df)[0]
...
# -------------------- DOCTOR CHATBOT (improved + debug) --------------------
    ...
    update_last_active()
    # yahan dobara set_page_config nahi bula rahe (sirf top pe ek baar)

    st.markdown(
        "<h2 style='text-align:center;'>🩺 MedGuardian — AI Doctor Chatbot</h2>",
        unsafe_allow_html=True,
    )

    use_gemini = st.checkbox(
        "Use Gemini (cloud LLM) for detailed answers (may require API key)", value=True
    )
    ...
    with st.form(key="chat_form", clear_on_submit=True):
        ...
        if send and user_input.strip() != "":
            ...
            # yahan safe_rerun() hata diya — ek hi click me message show ho jayega
